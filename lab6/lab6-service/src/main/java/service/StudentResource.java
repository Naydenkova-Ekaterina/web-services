...
public class StudentResource {
    ...
    public List<Student> getStudentsByName(@QueryParam("studentName") String name) throws IllegalFieldException {
        if (name == null || name.trim().isEmpty()) {
            throw IllegalFieldException.DEFAULT_INSTANCE;
        }
    ...
    @POST
    @Path("/createStudent")
    public Response createStudent(@QueryParam("studentName") String name, @QueryParam("studentSurname") String surname, @QueryParam("studentAge") int age, @QueryParam("studentGroupNumber") String groupNumber, @QueryParam("studentFaculty") String faculty) throws IllegalFieldException {
        PostgreSqlDAO dao = new PostgreSqlDAO();
        checkFieldsCorrection(name, surname, age, groupNumber, faculty);
        return Response
                .status(Response.Status.OK)
                .entity(dao.createStudent(name, surname, age, groupNumber, faculty).toString())
                .build();    }

    @PUT
    @Path("/updateStudent")
    public Response updateStudent(@QueryParam("id") Long id, @QueryParam("studentName") String name, @QueryParam("studentSurname") String surname, @QueryParam("studentAge") int age, @QueryParam("studentGroupNumber") String groupNumber, @QueryParam("studentFaculty") String faculty) throws NotExistException, IllegalFieldException {
        PostgreSqlDAO dao = new PostgreSqlDAO();
        checkExistence(id);
        checkFieldsCorrection(name, surname, age, groupNumber, faculty);
        ...
        return Response
                .status(Response.Status.OK)
                .entity(String.valueOf(dao.updateStudent(id, name, surname, age, groupNumber, faculty)))
                .build();    }

    @DELETE
    @Path("/deleteStudent")
    public Response deleteStudent(@QueryParam("id") Long id) throws NotExistException {
        PostgreSqlDAO dao = new PostgreSqlDAO();
        checkExistence(id);
        return Response
                .status(Response.Status.OK)
                .entity(String.valueOf(dao.deleteStudent(id)))
                .build();    }

    private void checkExistence(Long id) throws NotExistException {
        PostgreSqlDAO dao = new PostgreSqlDAO();
        if (dao.getStudentsById(id) == null) {
            throw new NotExistException("Student with selected id doesn't exist.");
        }
    }
    ...
    private void checkFieldsCorrection(String name, String surname, int age, String groupNumber, String faculty) throws IllegalFieldException {
        if (name == null || name.trim().isEmpty()) {
            throw new IllegalFieldException("studentName is not specified");
        } else if (surname == null || surname.trim().isEmpty()) {
            throw new IllegalFieldException("studentSurname is not specified");
        } else if (age <= 0 || age >= 150) {
            throw new IllegalFieldException("studentAge is not correct");
        } else if (groupNumber == null || groupNumber.trim().isEmpty()) {
            throw new IllegalFieldException("studentGroupNumber is not specified");
        } else if (faculty == null || faculty.trim().isEmpty()) {
            throw new IllegalFieldException("studentFaculty is not specified");
        }
    }
